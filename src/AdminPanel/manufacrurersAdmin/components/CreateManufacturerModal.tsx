...
export const CreateManufacturerModal:React.FC<CreateManufacturerModalProps> = ({createManufacturer, isOpen, onClose}) => {
    const formic = useFormik({
        ...
        onSubmit: values => {
            createManufacturer({id: 0, name: values.name, nameUK: values.nameUK});
            resetForm();
        },
    })
    ...
    return (
        <ModalWindow isOpen={isOpen} onClose={onClose}>
            <h3 className="text-center text-lg font-bold">Добавление нового производителя</h3>
            <form onSubmit={formic.handleSubmit} className="flex flex-col">
                <label className="mt-5" htmlFor="name">Имя производителя на русском</label>
                <input
                    id="name"
                    name="name"
                    type="text"
                ...
                <label className="mt-5" htmlFor="name">Имя производителя на украинском</label>
                <input
                    id="nameUK"
                    name="nameUK"
                    type="text"
